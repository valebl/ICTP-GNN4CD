...
if __name__ == '__main__':
    ...
    degree = degree(low_high_graph["high", "within", "high"].edge_index[1], low_high_graph["high"].num_nodes).cpu().numpy()
    ...
    if args.target_type=="precipitation":
        ...
        min_pr = min_pr_target.cpu().item()
        max_pr = max_pr_target.cpu().item()
        #pr= (pr_Rall*(max_pr_target-min_pr_target))+min_pr_target 
        pr = (pr_Rall*(max_pr-min_pr))+min_pr
        pr = pr[mask,:]
