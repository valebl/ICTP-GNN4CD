...
from utils.tools import prepare_target_Rall, prepare_target_T_Rall, find_not_all_nan_times_new, derive_train_val_idxs_new
from utils.tools import derive_qmse_bins, new_compute_input_statistics, new_standardize_input
...
if __name__ == '__main__':
    ...
    if "quantized" in args.loss_fn:
        loss_fn = getattr(loss_functions, args.loss_fn)()
    else:
    ...
    if args.target_var == "precipitation":
        write_log(f"the target is precipitation", args, accelerator, 'w')
        target_train = prepare_target_Rall(target_train, threshold=0.1)
    ...
    # Try 0201
    means_low, stds_low, means_high, stds_high = new_compute_input_statistics(
    ...
    low_high_graph['low'].x, low_high_graph['high'].x = new_standardize_input(
...
#-----------------------------------------------------
    ...
    write_log(f"\nAlpha value: {args.alpha}.", args, accelerator, 'a')
