...
class GNN4CD_model_GN(nn.Module):
    ''' The same architecture but with GraphNorm instead of BatchNorm
    '''
    def __init__(self, encoding_dim=128, seq_l=25, h_in=5*5, h_hid=5*5, n_layers=2, high_in=6+1, low2high_out=64, high_out=64):
        super(GNN4CD_model_GN, self).__init__()
        ...
        self.downscaler = geometric_nn.Sequential('x, edge_index', [
            (geometric_nn.GraphConv((encoding_dim, high_in), out_channels=low2high_out, dropout=0.2, heads=1, aggr='mean', add_self_loops=False, bias=True), 'x, edge_index -> x')
            ])
        
        self.processor = geometric_nn.Sequential('x, edge_index, batch', [
            (geometric_nn.GraphNorm(low2high_out), 'x, batch -> x'),
            ...
            (geometric_nn.GraphNorm(high_out*2), 'x, batch -> x'), 
            nn.ReLU(),
    ...
    def forward(self, data):   
        ...
        encod_high = self.processor(encod_low2high , data.edge_index_dict[('high','within','high')], data['high'].batch)
