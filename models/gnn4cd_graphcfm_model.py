...
class Processor(nn.Module):
    ...
    def forward(self, x, edge_index):
        return self.block1(x, edge_index)

class GraphCFMHead(nn.Module):
    def __init__(self, node_dim=64, time_emb_dim=32, hidden=32, heads=2, output_dim=1, dropout=0.1):
        """
        ...
        # layer 1 - neighbours communicate through x_t
        in_dim_1 = output_dim + time_emb_dim + node_dim
        self.gat1 = GATv2Conv(
            in_dim_1, hidden, heads=heads,
            dropout=dropout, aggr='add', add_self_loops=True,
        )
        self.norm1 = nn.LayerNorm(hidden * heads)
 
        # layer 2 - re-inject t_emb and node_emb
        in_dim_2   = hidden * heads + time_emb_dim + node_dim
        self.gat2  = GATv2Conv(
            in_dim_2, hidden, heads=heads,
            dropout=dropout, aggr='add', add_self_loops=True,
        )
    ...
    def forward(self, node_emb, x_t, t, edge_index):
        ...
        # layer 1
        inp1 = torch.cat([x_t, t_emb, node_emb], dim=-1)  # (N, in_dim_1)
        h    = F.silu(self.norm1(self.gat1(inp1, edge_index)))
 
        # layer 2 - re-inject conditioning
        inp2 = torch.cat([h, t_emb, node_emb], dim=-1)    # (N, in_dim_2)
        h2   = F.silu(self.norm2(self.gat2(inp2, edge_index)))

        return self.out(h2)                                 # (N, output_dim)
...
@register_model("GNN4CD_GraphCFM_Model")
class GNN4CD_GraphCFM_Model(nn.Module):
    """
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--cfm_hidden", type=int, default=32)
        parser.add_argument("--cfm_heads", type=int, default=2)
        parser.add_argument("--cfm_dropout", type=float, default=0.1)
    ...
    def _sample(self, encod_high, edge_index):
        # inference mode
        ...
        for _ in range(self.n_samples):
            ...
            for t_val in t_grid:
                v = self.cfm_head(encod_high, x, t_val.unsqueeze(0), edge_index)
                x = x + v * dt
            samples.append(x)
    ...
    def forward(self, data, inference=False):
        ...
        if y is not None:
            ...
            v_pred = self.cfm_head(encod_high, x_t, t, data.edge_index_dict[('high','within','high')])
