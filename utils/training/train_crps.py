import torch
import torch.nn as nn
import numpy as np
import pickle
import time
import wandb
from utils.metrics.metrics import AverageMeter
from utils.helpers.tools import write_log
from utils.plotting.plots import plot_maps, plot_pdf, get_cmap_dict
import matplotlib.pyplot as plt
import cartopy.crs as ccrs
import matplotlib
import json
from utils.helpers.tools import convert_dict
from torch_geometric.data import HeteroData


def spatial_gradient_loss(y_pred, y_true, edge_index, weight=0.1):
    """L1 loss on edge differences, encouraging spatial coherence."""
    src, dst = edge_index
    pred_diff = torch.abs(y_pred[src] - y_pred[dst])
    true_diff = torch.abs(y_true[src] - y_true[dst])
    return weight * torch.mean(torch.abs(pred_diff - true_diff))


#-----------------------------------------------------
#---------------------- TRAIN ------------------------
#-----------------------------------------------------

class CRPS_Trainer(object):

    def __init__(self, spatial_gradient_weight: float = 0.0,
                 val_ensemble_size: int = 1,
                 use_spectral: bool = False):
        """
        Parameters
        ----------
        spatial_gradient_weight : float
            Weight for the optional spatial-gradient regulariser on the
            ensemble mean. Set to 0.0 to disable.
        val_ensemble_size : int
            Number of noisy passes averaged at validation time when computing
            the MSE-on-mean monitoring metric. 1 = single sample (faster).
        use_spectral : bool
            If True, the loss is a CRPSSpectralLoss that requires preserved
            spatial structure. Target and members are reshaped to (B, N_high)
            and passed unmasked (the loss handles NaNs internally). If False,
            target and members are masked with train_mask and flattened to 1D
            before being passed to the (standard) CRPSLoss.
        """
        super().__init__()
        self.spatial_gradient_weight = spatial_gradient_weight
        self.val_ensemble_size = val_ensemble_size
        self.use_spectral = use_spectral

    def train_reg(
            self,
            model,
            dataloader_train,
            dataloader_val,
            optimizer,
            loss_fn,
            lr_scheduler,
            val_size,
            times,
            accelerator,
            args,
            epoch_start=0,
            log_val_plots=True):

        write_log(f"\nStart training the regressor with CRPS loss.", args, accelerator, 'a')

        mse_loss = nn.MSELoss()
        step = 0

        for epoch in range(epoch_start, epoch_start + args.epochs):

            model.train()
            write_log(f"\nEpoch {epoch} --- learning rate {optimizer.param_groups[0]['lr']:.8f}",
                      args, accelerator, 'a')

            loss_meter = AverageMeter()
            crps_meter = AverageMeter()
            grad_meter = AverageMeter()
            val_loss_meter = AverageMeter()

            start = time.time()

            # TRAIN
            for i, graph in enumerate(dataloader_train):

                train_mask = graph['high'].train_mask
                y = graph['high'].y

                # Train mode: model returns List[Tensor], one per noisy pass.
                members = model(graph)

                if self.use_spectral:
                    # Spectral CRPS needs preserved spatial structure.
                    # Reshape (B*N_high,) → (B, N_high); the loss handles NaNs
                    # internally (point-wise term filters them, spectral term
                    # replaces with 0 before FFT).
                    B = graph.num_graphs
                    N_high = y.shape[0] // B
                    y_for_loss = y.view(B, N_high)
                    members_for_loss = [m.squeeze().view(B, N_high) for m in members]
                    loss_crps = loss_fn(y_for_loss, members_for_loss)
                else:
                    # Standard CRPS: mask and flatten to 1D, like the existing
                    # NLL / MSE-QMSE-PSD trainers.
                    members_masked = [m.squeeze()[train_mask] for m in members]
                    y_masked = y[train_mask]
                    loss_crps = loss_fn(y_masked, members_masked)

                loss = loss_crps

                # Sample count for the running meters (number of valid points)
                n_for_meter = int(train_mask.sum().item())

                # Optional spatial-gradient regulariser on the ensemble mean
                if self.spatial_gradient_weight > 0.0:
                    edge_index = graph.edge_index_dict[('high', 'within', 'high')]
                    ensemble_mean = torch.stack(
                        [m.squeeze() for m in members], dim=0
                    ).mean(dim=0)
                    loss_grad = spatial_gradient_loss(
                        ensemble_mean, y, edge_index,
                        weight=self.spatial_gradient_weight
                    )
                    loss = loss + loss_grad
                    grad_meter.update(val=loss_grad.item(), n=n_for_meter)

                optimizer.zero_grad()
                accelerator.backward(loss)
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                optimizer.step()
                step += 1

                loss_meter.update(val=loss.item(), n=n_for_meter)
                crps_meter.update(val=loss_crps.item(), n=n_for_meter)

                log_dict = {
                    'epoch': epoch,
                    'train loss iteration': loss_meter.val,
                    'train loss avg': loss_meter.avg,
                    'train crps avg': crps_meter.avg,
                }
                if self.spatial_gradient_weight > 0.0:
                    log_dict['train spatial grad avg'] = grad_meter.avg
                accelerator.log(log_dict, step=step)

            end = time.time()

            # End of epoch
            current_lr = (np.mean(lr_scheduler.get_last_lr())
                          if lr_scheduler is not None and hasattr(lr_scheduler, 'get_last_lr')
                          else optimizer.param_groups[0]['lr'])
            accelerator.log({
                'epoch': epoch,
                'train loss avg': loss_meter.avg,
                'train crps avg': crps_meter.avg,
                'lr': current_lr,
            }, step=step)

            write_log(
                f"\nEpoch {epoch} completed in {end - start:.4f} seconds. "
                f"Loss - total: {loss_meter.sum:.4f} - average: {loss_meter.avg:.10f}. ",
                args, accelerator, 'a'
            )

            accelerator.save_state(
                output_dir=args.output_path + f"checkpoints/checkpoint_{epoch}/",
                safe_serialization=False
            )
            torch.save({"epoch": epoch}, args.output_path + f"checkpoints/checkpoint_{epoch}/epoch")

            # VALIDATION: monitor MSE between the (averaged) noisy ensemble mean
            # and the target. In eval mode model(graph) returns a single tensor.
            if dataloader_val is not None:
                model.eval()

                if log_val_plots:
                    y_pred_list = []
                    y_list = []
                    idxs_list = []

                with torch.no_grad():
                    for graph in dataloader_val:

                        train_mask = graph['high'].train_mask
                        y = graph['high'].y

                        if self.val_ensemble_size > 1:
                            samples = [model(graph).squeeze() for _ in range(self.val_ensemble_size)]
                            y_pred = torch.stack(samples, dim=0).mean(dim=0)
                        else:
                            y_pred = model(graph).squeeze()

                        loss = mse_loss(y_pred[train_mask], y[train_mask])
                        val_loss_meter.update(val=loss.item(), n=y_pred.shape[0])

                        accelerator.log({
                            'epoch': epoch,
                            'val loss iteration': val_loss_meter.val,
                            'val loss avg': val_loss_meter.avg,
                        }, step=step)

                        if log_val_plots:
                            n_nodes = graph['high'].num_nodes
                            B = y.shape[0] // n_nodes
                            y = y.view(B, n_nodes)
                            y_pred = y_pred.view(B, n_nodes)

                            y_pred = torch.atleast_2d(y_pred)
                            y = torch.atleast_2d(y)
                            idxs = torch.atleast_2d(torch.tensor(graph.idxs, device=accelerator.device))
                            y_pred_list.append(y_pred)
                            y_list.append(y)
                            idxs_list.append(idxs)

                    if log_val_plots:
                        y_pred_all = accelerator.gather(torch.stack(y_pred_list)).swapaxes(0, 1)[:, :val_size]
                        y_all = accelerator.gather(torch.stack(y_list)).swapaxes(0, 1)[:, :val_size]
                        idxs_all = accelerator.gather(torch.stack(idxs_list)).squeeze()[:val_size]

                        self._create_plots_reg(
                            y_pred_all, y_all, idxs_all, times, graph,
                            accelerator, step, epoch, args
                        )

                accelerator.log({
                    'epoch': epoch,
                    'val loss avg': val_loss_meter.avg,
                }, step=step)

            if lr_scheduler is not None:
                if isinstance(lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    lr_scheduler.step(val_loss_meter.avg)
                else:
                    lr_scheduler.step()

    def _create_plots_reg(self, y_pred, y, t, times, graph, accelerator, step, epoch, args,
                          binmax=1000,
                          metadata_file_path="/leonardo_work/ICT26_ESP/vblasone/GNN4CD-CORDEXML/utils/CORDEXML_plot_params.json"):

        with open(metadata_file_path) as f:
            meta = json.load(f)
        meta = convert_dict(meta)
        target_type = args.target_type

        lon = graph['high'].lon.cpu().numpy()
        lat = graph['high'].lat.cpu().numpy()

        _, indices = torch.sort(t)
        indices = indices.cpu().numpy()
        times = times[indices]

        y_pred_plot = y_pred.squeeze().swapaxes(0, 1).cpu().numpy()[:, indices]
        y_plot = y.squeeze().swapaxes(0, 1).cpu().numpy()[:, indices]

        if target_type == "precipitation":
            y_pred_plot = np.expm1(y_pred_plot)
            y_plot = np.expm1(y_plot)
        y_pred_pdf = y_pred_plot.flatten()
        y_pdf = y_plot.flatten()

        if target_type == "precipitation":
            bins = np.arange(0, binmax, 1).astype(np.float32)
        else:
            binmin = min(np.floor(np.min(y_pred_plot)), np.floor(np.min(y_plot))) - 5
            binmax = max(np.ceil(np.max(y_pred_plot)), np.ceil(np.min(y_plot))) + 5
            bins = np.arange(binmin, binmax, 1).astype(np.float32)

        cmap_dict = get_cmap_dict()
        pr_gnn4cd_avg = np.nanmean(y_pred_plot, axis=-1)
        pr_target_avg = np.nanmean(y_plot, axis=-1)
        bias = pr_gnn4cd_avg - pr_target_avg

        if meta[target_type]["cmap"] == "cmap_dict['avg']['cmap']":
            cmap = cmap_dict['avg']['cmap']
        else:
            cmap = meta[target_type]["cmap"]

        fig_avg = plot_maps(
            [lon, lon], [lat, lat],
            [pr_gnn4cd_avg, pr_target_avg],
            aggr=None, s=meta[target_type]["s"],
            legend_title=meta[target_type]["map_unit"], cmap=cmap,
            sub_titles=["GNN4CD (ensemble mean)", "TARGET"],
            x_size=meta["general"]["figsize"][0],
            y_size=meta["general"]["figsize"][1],
            font_size_title=meta["general"]["fontsize_title"],
            font_size=meta["general"]["fontsize"],
            cbar_title_size=meta["general"]["fontsize_cbar_title"],
            pr_max=meta[target_type]["vmax"], pr_min=meta[target_type]["vmin"],
            cbar_pad=20, suptitle_y=0.87, suptitle_x=0.72,
            show_ticks=False, plot_func="scatter",
            xlim=meta["general"]["xlim"], ylim=meta["general"]["ylim"],
            proj=ccrs.PlateCarree(), cbar_ax_lim=[0.93, 0.23, 0.015, 0.55]
        )

        fig_bias = plot_maps(
            lon, lat, bias, aggr=None, s=meta[target_type]["s"],
            legend_title=meta[target_type]["map_unit"], cmap="BrBG",
            sub_titles=["GNN4CD - TARGET"],
            x_size=meta["general"]["figsize"][0] / 2,
            y_size=meta["general"]["figsize"][1],
            font_size_title=25, font_size=20, cbar_title_size=20,
            pr_max=meta[target_type]["vmax_bias"],
            pr_min=meta[target_type]["vmin_bias"],
            cbar_pad=20, suptitle_y=0.87, suptitle_x=0.72,
            show_ticks=False, plot_func="scatter",
            xlim=meta["general"]["xlim"], ylim=meta["general"]["ylim"],
            proj=ccrs.PlateCarree(), cbar_ax_lim=[0.93, 0.23, 0.015, 0.55]
        )

        hist_vals, bins = np.histogram(y_pred_pdf, bins=bins, density=False)
        bins_mid = (bins[:-1] + bins[1:]) / 2
        Ntot = np.nansum(hist_vals)
        hist_vals_target, bins_target = np.histogram(y_pdf, bins=bins, density=False)
        bins_target_mid = (bins_target[:-1] + bins_target[1:]) / 2
        Ntot_target = np.nansum(hist_vals_target)

        if meta[target_type]["xlim_pdf"] is None:
            meta[target_type]["xlim_pdf"] = [0.2, bins.max() + 10]

        fig_pdf = plot_pdf(
            bin_list=[bins_target_mid, bins_mid],
            hist_list=[hist_vals_target / Ntot_target, hist_vals / Ntot],
            label_list=["TARGET", "GNN4CD"],
            xlabel=meta[target_type]["pdf_unit"],
            color_list=["black", "darkorange"],
            tail_lim=meta[target_type]["tail_lim"],
            ylim=meta[target_type]["ylim_pdf"],
            title=meta[target_type]["pdf_title"],
            xlim=meta[target_type]["xlim_pdf"],
            plot_func=meta[target_type]["plot_func_pdf"],
            fontsize=20, suptitle="",
            tail_ylim=meta[target_type]["tail_ylim"],
            log_xy=meta[target_type]["log_xy"],
            tail_zoom=meta[target_type]["tail_zoom"],
            legend_outside=meta[target_type]["legend_outside"]
        )

        accelerator.log({
            meta[target_type]["map_title"]: [wandb.Image(fig_avg)],
            "bias": [wandb.Image(fig_bias)],
            "pdf": [wandb.Image(fig_pdf)],
        }, step=step)

        plt.close(fig_avg)
        plt.close(fig_bias)
        plt.close(fig_pdf)

        if epoch == (args.epochs - 1):
            data = HeteroData()
            if args.target_type == "precipitation":
                data.pr_gnn4cd = y_pred_plot
            elif args.target_type == "temperature":
                data.tasmax_gnn4cd = y_pred_plot
            data.target = y_plot
            data.times = times
            data.times_target = times
            data["high"].lat = lat
            data["high"].lon = lon
            with open(args.output_path + f"output_graph_{args.validation_year}.pkl", 'wb') as f:
                pickle.dump(data, f)
