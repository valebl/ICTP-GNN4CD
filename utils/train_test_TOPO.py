...
def spatial_gradient_loss(y_pred, y_true, edge_index, weight=0.1):
    """
    Spatial gradient loss: penalizes overly smooth predictions

    """
    # calculate the difference between predictions of neighboring nodes
    src, dst = edge_index
    ...
    def train_R_Rall(self, model, dataloader_train, dataloader_val, optimizer, loss_fn, 
        ...
        for epoch in range(epoch_start, epoch_start+args.epochs):
            ...
            for graph in dataloader_train:
                ...
                #Try adding spatial gradient loss
            ...
            # Revised: log learning rate and support different schedulers
            if lr_scheduler is not None:
                if hasattr(lr_scheduler, 'get_last_lr'):
                    current_lr = np.mean(lr_scheduler.get_last_lr())
                else:
                    # ReduceLROnPlateau does not have get_last_lr()
            ...
            # Revised: support different schedulers
            if lr_scheduler is not None:
                if isinstance(lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    # ReduceLROnPlateau validation loss is used for stepping
