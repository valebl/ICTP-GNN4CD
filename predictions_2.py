...
from utils.testing.test_CRPS import CRPS_Tester
...
#-- CRPS / noisy-model parameters (used when model_name is a Noisy* variant)
...
parser.add_argument('--members_for_training', type=int, default=5,
                    help='members_for_training arg passed to the noisy model constructor (unused at inference)')
parser.add_argument('--ensemble_size', type=int, default=10,
                    help='number of noisy passes over the dataloader to build the test ensemble')
...
def _is_noisy_model(model_name: str) -> bool:
    return model_name is not None and "Noisy" in model_name
...
#--------------------------------------------------------
    ...
    if _is_noisy_model(args.model_name):
    ...
    sigma_pred = None
    ensemble_pred = None  # only populated for noisy/CRPS models
    ...
    elif _is_noisy_model(args.model_name):
        write_log(
            f"\nRunning CRPS ensemble inference with ensemble_size={args.ensemble_size}.",
            args, accelerator, 'a'
        )
        # ensemble_pred: (ensemble_size, n_test, N_high), idxs: (n_test,)
        ensemble_pred, idxs = CRPS_Tester().test_CRPS(
            model, dataloader, args=args,
            ensemble_size=args.ensemble_size,
            accelerator=accelerator,
        )
        # Use the ensemble mean as the deterministic prediction for the rest of
        # the post-processing path (mask/inversion/saving).
        y_pred = ensemble_pred.mean(dim=0)
    else:
    ...
    if accelerator is not None:
        ...
        if ensemble_pred is not None:
            # ensemble_pred: (ensemble_size, n_test, nodes). Gather only the
            # time axis across processes; the ensemble axis is local.
            gathered_members = []
            for m in range(ensemble_pred.shape[0]):
                m_gathered = accelerator.gather(ensemble_pred[m])[: len(dataset_graph), :]
                m_gathered = m_gathered.swapaxes(0, 1).cpu().numpy()[:, indices]  # (nodes, times)
                gathered_members.append(m_gathered)
            # (ensemble_size, nodes, times)
            ensemble_pred = np.stack(gathered_members, axis=0)
    ...
    if args.target_type == "precipitation":
        ...
        if ensemble_pred is not None:
            ensemble_pred = np.where(np.isfinite(np.expm1(ensemble_pred)),
                                     np.expm1(ensemble_pred), np.nan)
            ensemble_pred[ensemble_pred < THRESHOLD] = 0.0
    elif args.target_type == "temperature":
        y_pred = invert_normalization(y_pred, stats_path=args.train_path_R)
        if ensemble_pred is not None:
            inv_members = [
                invert_normalization(ensemble_pred[m], stats_path=args.train_path_R)
                for m in range(ensemble_pred.shape[0])
            ]
            ensemble_pred = np.stack(inv_members, axis=0)
    ...
    if ensemble_pred is not None:
        # (ensemble_size, nodes, times). Also store mean and spread for convenience.
        data.ensemble_gnn4cd = ensemble_pred
        data.ensemble_mean_gnn4cd = np.nanmean(ensemble_pred, axis=0)
        data.ensemble_spread_gnn4cd = np.nanstd(ensemble_pred, axis=0)
        data.ensemble_size = ensemble_pred.shape[0]
